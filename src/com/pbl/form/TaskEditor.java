...
/**
 * TaskEditor: Dialog để thêm/sửa/xóa Task.
 * Giữ nguyên giao diện gốc, chỉ bổ sung callback onUpdate để refresh danh sách.
 */
public class TaskEditor {
    ...
    public TaskEditor(Task t, MainForm mainForm, JPanel parentPanel, Runnable onUpdate) {
        ...
        if (t.getTitle() != null) frame.setTitle(t.getTitle());
        ...
        saveTaskButton.addActionListener(e -> {
            ...
            if (t.getID() != 0) taskService.updateTask(t);
            else taskService.createTask(t);
        ...
        deleteTaskButton.addActionListener(e -> {
            if (t.getID() != 0) taskService.deleteTask(t.getID());
            onUpdate.run();  // gọi callback
            frame.dispose();
        });
    }
