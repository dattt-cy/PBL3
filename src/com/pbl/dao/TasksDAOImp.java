...
public class TasksDAOImp implements TasksDAO {
    ...
    public void updateTask(Task t) {
        ...
        String update = "UPDATE task SET task_name = ?, description = ?, category = ?, status = ?, date = ? , sent_count = ? WHERE task_id = ?";
        try {
            String formattedDateTime = t.getDateTime().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
            dbHelper.executeUpdate(update, t.getTitle(), t.getDescription(), t.getCategory(), t.isDone(), formattedDateTime,t.getSentCount(), t.getID());
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
    ...
    @Override
    public void updateSentCount(Task t) {
         String sql = "UPDATE task SET sent_count = ? WHERE task_id = ?";
        try {
            dbHelper.executeUpdate(sql, t.getSentCount(), t.getID());
        } catch (SQLException ex) {
            Logger.getLogger(TasksDAOImp.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    @Override
    public int getTaskByID(int taskID) {
         String select = "SELECT sent_count FROM task WHERE task_id = ?";
        try (ResultSet rs = dbHelper.getRecords(select, taskID)) {
            if (rs.next()) {
                return rs.getInt("sent_count");
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
